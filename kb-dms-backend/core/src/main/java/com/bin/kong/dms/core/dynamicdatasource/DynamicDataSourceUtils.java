...
public class DynamicDataSourceUtils {
   ...
   * @return
   */
  private static DataSource createDataSource(String ip, int port, String db, String username, String password,
      Integer type) {
  ...
  private static String getConnectionUrlWithDb(String ip, Integer port, String db, Integer type) {
    switch (DatasourceTypeEnum.getByType(type)) {
      case MYSQL:
        return "jdbc:" + DatasourceTypeEnum.MYSQL.getName() + "://" + ip + ":" + port + "/" + db
            + "?useSSL=false&useUnicode=true&characterEncoding=utf8&tinyInt1isBit=false";
      case SQL_SERVER:
