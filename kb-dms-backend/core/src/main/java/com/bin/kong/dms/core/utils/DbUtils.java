...
public class DbUtils {
   ...
   * @return
   */
  public static Result<Connection> connectForMysql(String ip, Integer port, String username, String password) {
   ...
   * @return
   */
  public static Result<Connection> testConnect(String ip, Integer port, String username, String password,
      Integer type) {
  ...
  public static Result<Connection> connectForSqlServer(String ip, Integer port, String username, String password) {
  ...
  public static List<String> getTableNames(String ip, Integer port, String username, String password, String db,
      Integer type) {
  ...
  public static TableInfoEntity getTableInfo(Integer datasource_id, String ip, Integer port, String username,
      String password, String db, Integer type, String tableName) {
    ...
    try {
      ...
      String sql = "select * from information_schema.tables where table_schema='" + db + "'  and table_name='"
          + tableName + "' limit 1";
   ...
   * @return
   */
  public static List<TableFieldEntity> getColumnNames(Integer datasource_id, String ip, Integer port, String username,
      String password, String db, String tableName, Integer type) {
   ...
   * @return
   */
  public static List<TableIndexEntity> getTableIndex(Integer datasource_id, String ip, Integer port, String username,
      String password, String db, String tableName, Integer type) {
   ...
   * @return
   */
  public static Result<Connection> getConnection(String ip, Integer port, String username, String password,
      Integer type, String db) {
  ...
  private static boolean check_is_no_length(Integer type, String type_name) {
    ...
    if (type.equals(DatasourceTypeEnum.MYSQL.getType())) {
      if (Arrays.asList(MYSQL_DATE_TYPE_LIST).contains(type_name.toUpperCase())
          || Arrays.asList(MYSQL_DATE_TYPE_LIST).contains(type_name.toLowerCase())) {
        return true;
      }

      if (Arrays.asList(MYSQL_TEXT_TYPE_LIST).contains(type_name.toUpperCase())
          || Arrays.asList(MYSQL_TEXT_TYPE_LIST).contains(type_name.toLowerCase())) {
        return true;
      }
    }

    if (type.equals(DatasourceTypeEnum.SQL_SERVER.getType())) {
      if (Arrays.asList(SQL_NO_LENGTH_TYPE_LIST).contains(type_name.toUpperCase())
          || Arrays.asList(SQL_NO_LENGTH_TYPE_LIST).contains(type_name.toLowerCase())) {
        return true;
      }
    }
  ...
  private static List<TableIndexEntity> deal_meta_index_to_table_index(List<MetaIndexEntity> metaIndexEntityList) {
    ...
    for (String s : resultMap.keySet()) {
      ...
      if (!CollectionUtils.isEmpty(metaIndexEntities)) {
        metaIndexEntities = metaIndexEntities.stream()
            .sorted(Comparator.comparing(d -> d.getORDINAL_POSITION(), Comparator.naturalOrder()))
        ...
        indexEntityList.add(TableIndexEntity.builder()
            ...
            .online_index_name(s)
            .index_type(
                s.equals("PRIMARY") ? "PRIMARY" : (metaIndexEntities.get(0).getNON_UNIQUE() ? "INDEX" : "UNIQUE"))
            .build());
