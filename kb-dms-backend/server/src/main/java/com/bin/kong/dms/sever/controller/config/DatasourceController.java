...
public class DatasourceController extends BaseController {
  ...
  public GenericResponse ajax_datasource_search(@RequestParam(required = false) String kw,
      @RequestParam(required = false) Integer status, @RequestParam(required = false) String type) {
    ...
    try {
      ...
      if (StringUtils.isNotEmpty(type)
          && type.toUpperCase().equals(DatasourceSearchTypeEnum.OWNER_BY_ME.getName().toUpperCase())) {
        DatasourceJoinGroupJoinOwnerSearch datasourceJoinGroupJoinOwnerSearch = DatasourceJoinGroupJoinOwnerSearch
            .builder()
            .owner(super.getUserInfoDTO().getAccount())
            .kw(kw)
            .status(status)
            .build();
        List<DatasourceJoinGroupJoinOwner> datasourceJoinGroupJoinOwnerList = datasourceJoinGroupJoinOwnerMapper
            .searchList(datasourceJoinGroupJoinOwnerSearch);
        ...
        List<DatasourceJoinGroup> datasourceJoinGroupList = datasourceJoinGroupMapper
            .searchList(datasourceJoinGroupSearch);
    ...
    try {
      ...
      if (null != datasource) {
        if (datasource.getStatus().equals(DataSourceStatusEnum.NO_ACTIVE.getStatus())) {
          Result result = DbUtils.getConnection(datasource.getIp(), datasource.getPort(), datasource.getUsername(),
              PPAesUtils.decodeAES(datasource.getPassword()), datasource.getType());
  ...
  public GenericResponse ajax_get_datasource_db(@PathVariable Integer id) {
    ...
    try {
      CfDatasource datasource = datasourceMapper.selectByPrimaryKey(id);
      List<String> dbList = DbUtils.getDbNames(datasource.getIp(), datasource.getPort(), datasource.getUsername(),
          PPAesUtils.decodeAES(datasource.getPassword()), datasource.getType());
        ...
        if (CollectionUtils.isNotEmpty(favoriteDbList)) {
          dbResponseList = dbResponseList.stream().map(d -> {
            if (CollectionUtils.isNotEmpty(
                favoriteDbList.stream().filter(f -> f.getDb().equals(d.getName())).collect(Collectors.toList()))) {
              d.setFavorite(true);
            }
            return d;
      ...
      dbResponseList = dbResponseList.stream()
          .sorted(
              Comparator.comparing(r -> null != r.getFavorite() && r.getFavorite() ? 1 : 0, Comparator.reverseOrder()))
          .collect(Collectors.toList());
  ...
  public GenericResponse ajax_get_datasource_db_table(@PathVariable("id") Integer id, @PathVariable("db") String db) {
    ...
    try {
      CfDatasource datasource = datasourceMapper.selectByPrimaryKey(id);
      List<String> tableList = DbUtils.getTableNames(datasource.getIp(), datasource.getPort(), datasource.getUsername(),
          PPAesUtils.decodeAES(datasource.getPassword()), db, datasource.getType());
      ...
      if (CollectionUtils.isNotEmpty(tableResponseList)) {
        ...
        if (CollectionUtils.isNotEmpty(favoriteTableList)) {
          tableResponseList = tableResponseList.stream().map(d -> {
            if (CollectionUtils.isNotEmpty(favoriteTableList.stream().filter(f -> f.getTable_name().equals(d.getName()))
                .collect(Collectors.toList()))) {
              d.setFavorite(true);
            }
            return d;
      ...
      tableResponseList = tableResponseList.stream()
          .sorted(
              Comparator.comparing(r -> null != r.getFavorite() && r.getFavorite() ? 1 : 0, Comparator.reverseOrder()))
          .collect(Collectors.toList());
  ...
  public GenericResponse ajax_get_datasource_db_table_filed(@PathVariable("id") Integer id,
      @PathVariable("db") String db, @PathVariable("table") String table) {
    ...
    try {
      ...
      List<TableFieldEntity> fieldList = DbUtils.getColumnNames(datasource.getId(), datasource.getIp(),
          datasource.getPort(), datasource.getUsername(), PPAesUtils.decodeAES(datasource.getPassword()), db, table,
          datasource.getType());
  ...
  public GenericResponse ajax_get_datasource_db_table_info(@PathVariable("id") Integer id,
      @PathVariable("db") String db, @PathVariable("table") String table) {
    ...
    try {
      ...
      List<TableIndexEntity> indexEntityList = DbUtils.getTableIndex(datasource.getId(), datasource.getIp(),
          datasource.getPort(), datasource.getUsername(), PPAesUtils.decodeAES(datasource.getPassword()), db, table,
          datasource.getType());

      TableInfoEntity tableInfoEntity = DbUtils.getTableInfo(datasource.getId(), datasource.getIp(),
          datasource.getPort(), datasource.getUsername(), PPAesUtils.decodeAES(datasource.getPassword()), db,
          datasource.getType(), table);
  ...
  private List<DatasourceForSearchResponse> datasource_favorite_deal(List<DatasourceForSearchResponse> responseList) {
    ...
    List<UsFavoriteDatasource> favoriteDatasourceList = favoriteDatasourceMapper
        .selectList(UsFavoriteDatasource.builder()
            .account(super.getUserInfoDTO().getAccount())
            .build());

    if (CollectionUtils.isNotEmpty(favoriteDatasourceList)) {
      responseList.stream().map(d -> {
        if (CollectionUtils.isNotEmpty(favoriteDatasourceList.stream()
            .filter(f -> f.getDatasource_id().equals(d.getId())).collect(Collectors.toList()))) {
    ...
    responseList = responseList.stream()
        .sorted(Comparator.comparing(r -> null != r.getDatasource_favorite() && r.getDatasource_favorite() ? 1 : 0,
    ...
    if (CollectionUtils.isNotEmpty(groupOwnersList)) {
      responseList.stream().map(d -> {
        if (CollectionUtils.isNotEmpty(groupOwnersList.stream().filter(f -> f.getBus_group_id().equals(d.getGroup_id()))
            .collect(Collectors.toList()))) {
    ...
    if (CollectionUtils.isNotEmpty(groupUsersList)) {
      responseList.stream().map(d -> {
        if (CollectionUtils.isNotEmpty(groupUsersList.stream().filter(f -> d.getGroup_id().equals(f.getBus_group_id()))
            .collect(Collectors.toList()))) {
          d.setIs_my_group(true);
          return d;
        } else {
    ...
    responseList = responseList.stream().sorted(
        Comparator.comparing(r -> null != r.getIs_my_group() && r.getIs_my_group() ? 1 : 0, Comparator.reverseOrder()))
        .collect(Collectors.toList());
    ...
    if (CollectionUtils.isNotEmpty(favoriteGroupList)) {
      responseList.stream().map(d -> {
        if (CollectionUtils.isNotEmpty(favoriteGroupList.stream().filter(f -> f.getGroup_id().equals(d.getGroup_id()))
            .collect(Collectors.toList()))) {
    ...
    responseList = responseList.stream().sorted(Comparator
        .comparing(r -> null != r.getGroup_favorite() && r.getGroup_favorite() ? 1 : 0, Comparator.reverseOrder()))
        .collect(Collectors.toList());
