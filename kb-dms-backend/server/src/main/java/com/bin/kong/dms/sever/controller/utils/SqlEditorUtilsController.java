...
public class SqlEditorUtilsController extends BaseController {
  ...
  public GenericResponse ajax_get_table_create_sql(@PathVariable("id") Integer id, @PathVariable("db") String db,
    ...
    try {
      ...
      response.setData(getTableCreateSql(datasource.getId(), datasource.getIp(), datasource.getPort(),
          datasource.getUsername(), PPAesUtils.decodeAES(datasource.getPassword()), db, datasource.getType(), table));
  ...
  private String getTableCreateSql(Integer datasource_id, String ip, Integer port, String username, String password,
      String db, Integer type, String table) {

    List<TableFieldEntity> fieldList = DbUtils.getColumnNames(datasource_id, ip, port, username, password, db, table,
        type);
    ...
    for (TableFieldEntity tableFieldEntity : fieldList) {
      sql += ("    `" + tableFieldEntity.getColumn_name()
          ...
          + (((StringUtils.isNotEmpty(tableFieldEntity.getColumn_def())
              && tableFieldEntity.getColumn_def().length() > 0)
              || null != tableFieldEntity.getColumn_def() && tableFieldEntity.getColumn_def().equals(""))
                  ? tableFieldEntity.getType_name().toUpperCase().indexOf("DATETIME") != -1
                      && tableFieldEntity.getColumn_def().toUpperCase().indexOf("CURRENT_TIMESTAMP") != -1
                          ? " DEFAULT " + tableFieldEntity.getColumn_def()
                          : " DEFAULT '" + tableFieldEntity.getColumn_def() + "'"
                  : "")
    ...
    List<TableFieldEntity> primaryKeys = fieldList.stream().filter(f -> f.getIs_primary_key())
    ...
    // 拼接索引
    List<TableIndexEntity> indexEntityList = DbUtils.getTableIndex(datasource_id, ip, port, username, password, db,
        table, type);

    if (!CollectionUtils.isEmpty(indexEntityList)) {
      List<TableIndexEntity> filter_index_rows = indexEntityList.stream()
          .filter(idx -> StringUtils.isNotEmpty(idx.getIndex_name()) && !idx.getIndex_name().equals("PRIMARY"))
          .collect(Collectors.toList());

      for (int i = 0; i < filter_index_rows.size(); i++) {
        if (filter_index_rows.get(i).getIndex_type().equals("UNIQUE")) {
          sql += "    UNIQUE KEY `" + filter_index_rows.get(i).getIndex_name() + "` ("
              + get_index_columns(filter_index_rows.get(i).getIndex_columns()) + "),\n";
        } else {
          sql += "    KEY `" + filter_index_rows.get(i).getIndex_name() + "` ("
              + get_index_columns(filter_index_rows.get(i).getIndex_columns()) + "),\n";
        }
      }
    }
    ...
    // 拼接表引擎及备注
    TableInfoEntity tableInfoEntity = DbUtils.getTableInfo(datasource_id, ip, port, username, password, db, type,
        table);
    if (null != tableInfoEntity && null != tableInfoEntity.getEngine()) {
      sql += (") ENGINE=" + tableInfoEntity.getEngine() + " DEFAULT CHARSET=" + tableInfoEntity.getCharacter_set()
          + " COMMENT='" + (StringUtils.isNotEmpty(tableInfoEntity.getCommit()) ? tableInfoEntity.getCommit() : "")
          + "'");
    }
