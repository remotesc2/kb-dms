...
public class SqlOptionsTypePermissionServiceImpl implements ISqlOptionsTypePermissionService {
  ...
  @Override
  public Result<Object> check(DynamicSqlOptionTypeEntity sqlOptionTypeEntity, String account, Integer datasource_id) {
    ...
    List<String> ownerList = datasourceOwnersList.stream().map(CfDatasourceOwners::getName)
    ...
    List<CfDatasourcePermissionSqlOptions> permissionSqlOptionsList = permissionSqlOptionsMapper
        .selectList(CfDatasourcePermissionSqlOptions.builder()
            .datasource_id(datasource_id)
            .account(account)
            .build());
